...
const IPDR = require('./models/IPDR');
...
// Add Evidence Log
app.put('/api/cases/:id/logs', upload.single('attachment'), async (req, res) => {
    try {
        console.log('Received log upload request for Case ID:', req.params.id);
        const { date, mode, remark, referenceData, criminalName, criminalAddress, criminalPhoto } = req.body;
        ...
        console.log('Log data received:', { date, mode, remark, referenceData, attachment });

        const caseItem = await Case.findById(req.params.id);
        if (!caseItem) return res.status(404).json({ message: 'Case not found' });

        caseItem.evidenceLogs.push({ date, mode, remark, referenceData, attachment, criminalName, criminalAddress, criminalPhoto });
...
// Delete Evidence Log
app.delete('/api/cases/:id/logs/:logId', async (req, res) => {
    try {
...
app.post('/api/cdr', async (req, res) => {
    try {
...
// Delete CDR record
app.delete('/api/cdr/:id', async (req, res) => {
    try {
...
app.post('/api/cdr/upload/:id', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'No file uploaded' });
        }
...
app.get('/api/cdr/download/:id', async (req, res) => {
    try {
...
// Get All IPDRs
app.get('/api/ipdr', async (req, res) => {
    try {
...
app.post('/api/ipdr', async (req, res) => {
    try {
...
// Delete IPDR record
app.delete('/api/ipdr/:id', async (req, res) => {
    try {
...
app.post('/api/ipdr/upload/:id', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'No file uploaded' });
        }
...
app.get('/api/ipdr/download/:id', async (req, res) => {
    try {
...
app.delete('/api/social-media', async (req, res) => {
    try {
...
app.post('/api/social-media/upload', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'No file uploaded' });
        }
